...
- Card presets use stable semantic names: `default`, `classic`, `overlay`, `split`, `frosted`, `reveal`, `profile`, `metric`, and `article`. `default` is the implicit simple title-and-body layout; `classic` preserves the established media-first preset. Numeric values 1-5 are compatibility aliases only and must continue mapping to `classic`, `overlay`, `split`, `frosted`, and `reveal` without rendering numeric CSS classes.
- Card `texture` and `effect` are independent from `type` and from each other. `texture="default"` keeps the normal solid surface; texture layers change material appearance, while effect layers add interactive or edge decoration. Preserve the selected layout, slots, content interaction, and accessibility semantics when either or both are used.
- `texture="liquid-glass"` must provide observable optical displacement of the backdrop, not only transparency, gradients, or blur. Keep SVG filter identifiers unique and SSR-stable per Card instance, and retain a readable blur fallback when URL backdrop filters are unavailable.
- `texture="liquid-glass-2"` is the alternate specular SVG filter comparison. It must reuse the same Card layout, title, tint, shine, blur, border, and content treatment as `liquid-glass`; only the filter graph and the required filter application path may differ.
...
- Add `orientation`, `hover-effect`, `shape`, and interaction state as explicit opt-in extensions. Do not restore Card `variant`; its separate surface-style axis and examples were removed. An existing Card that does not pass an extension prop must retain its previous classes, DOM structure, layout, and hover behavior.
- Apply Card horizontal orientation across every named `type`: media sits beside content while preset-specific presentation remains recognizable, and `profile` keeps its avatar treatment. Do not disable or silently ignore horizontal orientation for media-first presets. Media may inherit only the outer corners it touches, and `shape="square"` must remain square even during hover.
...
- In Card documentation, keep one level-two heading inside each example card, matching other component pages. Use localized `EXAMPLE_GROUPS` frontmatter to group the `type`, `texture`, and `effect` examples in the sidebar without adding a second visible heading inside any example card. Keep the English and Chinese heading order and canonical slugs aligned.
- Keep one comprehensive Card configurator as the final documentation example. It must expose all independent visual and interaction axes together instead of maintaining a separate fixed texture/effect composition example.
- `SCardGroup` is removed. Do not restore Card-specific scrolling/group composition; use Carousel when multiple cards need paging, scrolling, or presentation as a collection.
