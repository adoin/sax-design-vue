...
`$buttons` accepts action records through `options`. `props.maxVisible` keeps a chosen number inline and moves remaining actions into a shared `SPopper`; `props.trigger` accepts `click` or `hover`. Action state may be static or derived from renderer context. In Form, action codes `submit` and `reset` use the current Form methods when no explicit event handler is supplied. Table registers `button`, `$refresh`, `$columnConfig`, and `$find` for toolbar use. `button` accepts a direct action or child actions through `props`; `$columnConfig` mounts `STableColumnConfig`, while `$find` mounts an icon trigger that opens a find-and-replace panel in `SPopper` against the current Table context. Pass `content` or `props.content` to append a label. Each open resets the form and previous matches. A successful search collapses the panel to result-navigation controls; expand restores the form, and close or Escape dismisses it.
