...
export function createTableWork(options: TableWork) {
  const check = () => {
    if (options.signal?.aborted || options.current?.() === false)
      throw new DOMException('Table operation cancelled', 'AbortError')
  }
  let visited = 0
  ...
  const checkpoint = (): Promise<void> | undefined => {
    ...
    return new Promise<void>((resolve, reject) => {
      const abort = () => {
        cleanup()
        reject(new DOMException('Table operation cancelled', 'AbortError'))
      }
