...
/** Default text conversions follow built-in editors; custom data uses parseCell. */
export function parseTableClipboardValue(
  value: unknown,
  ...
  if (editor?.type === 'number') {
    ...
    if (!Number.isFinite(number))
      throw new TypeError('Clipboard number is not finite')
    return number
  }
  if (editor?.type === 'switch') {
    if (value === 'true') return true
    if (value === 'false') return false
    throw new TypeError('Clipboard switch value must be true or false')
  }
  ...
  if (editor?.type === 'date' && context.value instanceof Date) {
    ...
    if (!Number.isFinite(date.getTime()))
      throw new TypeError('Clipboard date is invalid')
    return date
  }
  return value
}
