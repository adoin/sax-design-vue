export default {
  name: 'zh-cn',
  vs: {
    common: {
      close: '关闭',
      more: '展开',
      less: '收起',
    },
    textEllipsis: {
      expand: '展开全文',
      collapse: '收起全文',
    },
    contentEllipsis: {
      expand: '展开内容',
      collapse: '收起内容',
    },
    colorpicker: {
      confirm: '确定',
      clear: '清空',
      defaultLabel: '颜色选择器',
      description: '当前颜色为 {color}。按 Enter 键选择新颜色。',
      color: '颜色',
      value: '颜色值',
      opacity: '透明度',
      choose: '选择 {color}',
      hue: '色相',
      format: '颜色格式',
      saturationValue: '饱和度和明度',
      eyedropper: '吸取屏幕颜色',
      eyedropperFailed: '未能吸取屏幕颜色。',
    },
    datepicker: {
      now: '此刻',
      today: '今天',
      cancel: '取消',
      clear: '清空',
      confirm: '确定',
      selectDate: '选择日期',
      selectTime: '选择时间',
      dateTablePrompt: '使用方向键和 Enter 键选择日期',
      monthTablePrompt: '使用方向键和 Enter 键选择月份',
      yearTablePrompt: '使用方向键和 Enter 键选择年份',
      selectedDate: '已选择日期',
      startDate: '开始日期',
      startTime: '开始时间',
      endDate: '结束日期',
      endTime: '结束时间',
      prevYear: '前一年',
      nextYear: '后一年',
      prevMonth: '上个月',
      nextMonth: '下个月',
      year: '年',
      month1: '1 月',
      month2: '2 月',
      month3: '3 月',
      month4: '4 月',
      month5: '5 月',
      month6: '6 月',
      month7: '7 月',
      month8: '8 月',
      month9: '9 月',
      month10: '10 月',
      month11: '11 月',
      month12: '12 月',
      // week: '周次',
      weeks: {
        sun: '日',
        mon: '一',
        tue: '二',
        wed: '三',
        thu: '四',
        fri: '五',
        sat: '六',
      },
      months: {
        jan: '一月',
        feb: '二月',
        mar: '三月',
        apr: '四月',
        may: '五月',
        jun: '六月',
        jul: '七月',
        aug: '八月',
        sep: '九月',
        oct: '十月',
        nov: '十一月',
        dec: '十二月',
      },
    },
    calendar: {
      previousMonth: '上个月',
      nextMonth: '下个月',
      today: '今天',
      weekNumber: '周',
      allDay: '全天',
      events: '个日程',
      moreEvents: '查看当天全部日程',
      editEvent: '编辑日程',
      deleteEvent: '删除日程',
      views: {
        month: '月',
        week: '周',
        day: '日',
      },
    },
    timepicker: {
      input: '时间输入',
      hours: '时',
      minutes: '分',
      seconds: '秒',
    },
    backtop: {
      label: '返回顶部',
    },
    anchor: {
      navigation: '锚点导航',
      expand: '展开锚点分组',
      collapse: '收起锚点分组',
      previousRoute: '上一章节',
      nextRoute: '下一章节',
      continueScrollUp: '继续向上滚动即可进入',
      continueScroll: '继续向下滚动即可进入',
      reachTop: '滚动到页面顶部后继续',
      reachBottom: '滚动到页面底部后继续',
      routeLoading: '正在打开下一章节',
      scrollProgress: '继续滚动进度',
    },
    breadcrumb: {
      label: '面包屑导航',
      hiddenLevels: '已省略的层级',
      fullTrail: '完整面包屑',
      closeFullTrail: '关闭完整面包屑',
      expandChildren: '展开子级位置',
    },
    layout: {
      collapseOutside: '收起外挂工具',
      expandOutside: '展开外挂工具',
    },
    menu: {
      label: '菜单',
    },
    prompt: {
      accept: '确定',
      cancel: '取消',
      title: '提示',
    },
    noticeBar: {
      close: '关闭',
    },
    carousel: {
      previous: '上一张',
      next: '下一张',
      goto: '跳转到第 {index} 张',
    },
    imagePreview: {
      label: '图片预览',
      close: '关闭',
      previous: '上一张图片',
      next: '下一张图片',
      toolbar: '图片预览工具栏',
      zoomIn: '放大',
      zoomOut: '缩小',
      rotateLeft: '向左旋转',
      rotateRight: '向右旋转',
      fitToScreen: '适应窗口',
      originalSize: '显示原始尺寸',
      reset: '重置图片',
    },
    select: {
      loading: '加载中',
      noMatch: '无匹配数据',
      noData: '无数据',
      placeholder: '请选择',
      search: '搜索选项',
      selectAll: '全选',
      invert: '反选',
      clear: '清空',
      pin: '置顶选项',
      unpin: '取消置顶',
      selectedSummary: '已选 {count} 项：{labels}',
      selectedCount: '已选 {count} 项',
      visibleCount: '当前显示 {count} 项',
      totalCount: '共 {count} 项',
      pinHint: '悬停选项后点击置顶按钮，或按 Ctrl+P。',
      pinned: '已置顶',
      unpinned: '未置顶',
    },
    cascader: {
      noMatch: '无匹配数据',
      loading: '加载中',
      placeholder: '请选择',
      noData: '暂无数据',
      clear: '清空选择',
    },
    pagination: {
      goto: '前往',
      pagesize: '条/页',
      total: '共 {total} 条',
      previous: '上一页',
      next: '下一页',
      pageClassifier: '页',
      deprecationWarning:
        '你使用了一些已被废弃的用法，请参考 pagination 的官方文档',
    },
    iconPicker: {
      title: '插入图标',
      description: '选择图标、颜色和尺寸，生成需要的图标数据。',
      search: '搜索图标',
      empty: '未找到图标',
      emptyHint: '请尝试其他名称或图标集前缀',
      clearSearch: '清空搜索',
      options: '可选图标',
      resultCount: '{count} 个图标',
      noSelection: '尚未选择图标',
      color: '图标颜色',
      size: '图标尺寸',
      sizeHint: '可选范围 8–256 px',
      confirm: '确认选择',
      insert: '插入 SVG',
      cancel: '取消',
    },
    splitter: {
      resize: '调整{direction}面板大小',
    },
    steps: {
      label: '步骤',
      status: {
        wait: '等待中',
        process: '进行中',
        finish: '已完成',
        success: '成功',
        error: '需要处理',
        loading: '加载中',
        disabled: '不可用',
      },
    },
    tabs: {
      label: '标签页',
      more: '更多标签页',
      add: '添加标签页',
      close: '关闭{label}',
    },
    rate: {
      label: '评分',
    },
    messagebox: {
      title: '提示',
      confirm: '确定',
      cancel: '取消',
      error: '输入的数据不合法!',
    },
    dialog: {
      close: '关闭对话框',
      confirm: '确定',
      cancel: '取消',
    },
    alert: {
      close: '关闭提示',
    },
    notification: {
      close: '关闭通知',
    },
    upload: {
      deleteTip: '按 delete 键可删除',
      delete: '删除',
      preview: '查看图片',
      continue: '继续上传',
      uploadFile: '上传文件',
      limitReached: '已达到文件数量上限',
      dropHint: '点击选择，或将文件拖放到这里',
      queue: '上传队列',
      files: '个文件',
      start: '全部上传',
      ready: '等待上传',
      uploading: '上传中',
      success: '上传成功',
      error: '上传失败',
      retry: '重试',
      maxSize: '单个最大',
      maxFiles: '最多文件',
      invalidType: '不支持此文件类型',
      tooLarge: '文件超过允许的大小',
      tooMany: '已达到文件数量上限',
      rejected: '文件未通过校验',
    },
    table: {
      pendingChanges: '请先保存或还原待提交变更。',
      pendingDraft: '请先应用或放弃当前草稿。',
      invalidChanges: '请修正无效字段后再保存。',
      requestRejected: '数据所有者或服务拒绝了此次请求。',
      staleResult: '请求期间数据已变化，当前数据已保留。',
      unsupportedSource: '此操作需要生成数据源适配器及有效的校验范围。',
      requestFailed: '请求失败，请重试。',
      query: '查询',
      reset: '重置',
      queryMore: '更多',
      queryLess: '收起',
      refresh: '刷新',
      toolbar: '表格工具栏',
      rangeSelected: '已选择 {rows} 行、{columns} 列的区域。',
      rangeEmpty: '未选择单元格区域。',
      chartTitle: '图表',
      chartCategory: '类别',
      chartClose: '关闭',
      chartType_bar: '柱状图',
      chartType_line: '折线图',
      chartData: '查看数据',
      chartRendering: '正在绘制图表…',
      chartError: '图表绘制失败，仍可查看数据。',
      chartEmpty: '暂无图表数据。',
      chartAlternative: '可选择“查看数据”阅读完整的数据表格。',
      chartSeries: '数据系列',
      findTitle: '查找与替换',
      findText: '查找内容',
      findReplacement: '替换为',
      findScope: '搜索范围',
      findScope_view: '当前视图',
      findScope_selection: '选中区域',
      findScope_data: '已提供数据',
      findCase: '区分大小写',
      findWhole: '匹配整个单元格',
      findSearch: '查找',
      findPrevious: '上一个',
      findNext: '下一个',
      findReplace: '替换当前',
      findReplaceAll: '全部替换',
      findCancel: '取消操作',
      findClose: '关闭',
      findExpand: '展开查找条件',
      findWorking: '正在处理…',
      findCount: '匹配单元格 {current} / {total}',
      findLimited:
        '已检查 {visited} 个位置，找到 {count} 个匹配格。已达到上限，请缩小范围后使用全部替换。',
      findUnavailable: '当前筛选或受控视图无法显示此匹配。',
      findReplaced: '已更新 {count} 个单元格。',
      findReason_disabled: '替换前请开启查找与数据变更功能。',
      findReason_editing: '请先完成当前编辑。',
      findReason_cancelled: '操作已取消。',
      findReason_invalid: '搜索范围或替换值无效。',
      findReason_conflict: '源数据已变化，请重新查找。',
      findReason_empty: '请先选择一个匹配单元格。',
      findReason_readonly: '匹配单元格为只读。',
      findReason_validation: '校验未通过，源数据已保留。',
      findReason_limit: '已达到处理上限，请缩小范围。',
      findReason_busy: '另一个数据更新正在等待完成。',
      findReason_rejected: '数据所有者未接受本次更新。',
      clipboardWorking: '正在处理剪贴板操作。',
      clipboardCopied: '已复制单元格区域。',
      clipboardCut: '已剪切单元格区域。',
      clipboardPasted: '已粘贴剪贴板数据。',
      clipboardCancelled: '已取消剪贴板操作。',
      clipboardFailed: '剪贴板操作未能完成。',
      selectedCell: '位于选区内',
      dragRow: '调整第 {row} 行顺序',
      dragRowHint: '空格开始，方向键选择位置，回车放置，Escape 取消。',
      dragRowTarget: '放到第 {row} 行{position}，目标为第 {level} 层。',
      dragBefore: '之前',
      dragInside: '内部',
      dragAfter: '之后',
      dragStatus: {
        grabbed: '已拾取行。',
        moved: '行顺序已更新。',
        cancelled: '已取消调整顺序。',
        rejected: '行顺序未改变。',
      },
      validationRequired: '请填写{field}',
      validationInvalid: '{field}不符合规则',
      validationShowError: '查看错误：{message}',
      validationNavigation: '表格校验错误导航',
      validationPosition: '第 {current} / {total} 处',
      validationPrevious: '上一个错误',
      validationNext: '下一个错误',
      validationClose: '关闭错误导航',
      expandDetails: '展开第 {row} 行详情',
      collapseDetails: '收起第 {row} 行详情',
      rowDetails: '第 {row} 行详情',
      loadingDetails: '正在加载详情…',
      detailLoadFailed: '详情加载失败。',
      retryDetails: '重试',
      columnSettings: '列设置',
      columnChildren: '{count} 列',
      unfixedColumn: '不固定',
      fixedLeftColumn: '左侧固定',
      fixedRightColumn: '右侧固定',
      pinColumn: '{column} 的固定位置',
      moveColumnUp: '上移 {column}',
      moveColumnDown: '下移 {column}',
      dragColumn: '调整{column}列顺序',
      dragColumnHint:
        '拖动调整顺序或移入分组；键盘操作时按空格拾取，上下键选择行，右键移入分组，左键移出当前分组，再按回车放置或按 Escape 取消。',
      dragColumnTarget: '移动到第 {position} 个位置。',
      dragColumnInside: '移入{column}。',
      columnDragStatus: {
        grabbed: '已拾取列。',
        moved: '列顺序已更新。',
        cancelled: '已取消调整列顺序。',
      },
      resetColumns: '重置列',
      resizeColumn: '调整 {column} 列宽',
      sortAscending: '升序排序',
      sortDescending: '降序排序',
      clearSort: '取消排序',
      filter: '筛选',
      selectAll: '选择所有可选行',
      selectRow: '选择第 {row} 行',
      parentContext: '上级',
      returnToParent: '回到上级行',
      returnToParentLabel: '回到上级 {parent}',
      emptyText: '暂无数据',
      confirmFilter: '筛选',
      resetFilter: '重置',
      clearFilter: '全部',
      sumText: '合计',
    },
    tree: {
      emptyText: '暂无数据',
      expand: '展开节点',
      collapse: '收起节点',
    },
    transfer: {
      noMatch: '无匹配数据',
      noData: '无数据',
      titles: ['列表 1', '列表 2'],
      filterPlaceholder: '请输入搜索内容',
      noCheckedFormat: '共 {total} 项',
      hasCheckedFormat: '已选 {checked}/{total} 项',
    },
    image: {
      error: '加载失败',
    },
    pageHeader: {
      title: '返回',
    },
    popconfirm: {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
    },
  },
}
