...
`app.use(VuesaxAlpha)` 之后，可直接用 **`$prompt`**（Options API）或 **`VsPromptBox`**（按需导入），无需写模板。对应官方 `$vs.dialog()` 的 JS 唤起方式。
...
// 警告 — 关闭后 resolve
await VsPromptBox.alert('保存成功', '提示')

// 确认 — 点确定 resolve，取消或关闭 reject
try {
  await VsPromptBox.confirm('确定删除该项？', '确认')
  // 用户确认
} catch {
  // 用户取消
}
