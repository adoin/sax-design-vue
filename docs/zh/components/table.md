...
数据会重排、更新或包含树节点时，建议提供稳定唯一的 `row-key`。横向滚动时，行高会保留已显示内容的最大高度，以减少上下跳动。列宽、容器宽度、列显隐或顺序变化后会重新测量；替换 `data` 数组或 `virtualSource.row` 回调也会清除旧的行高测量。

原地修改行字段、自定义插槽内容或回调所读取的外部数据时，可见内容增高会自动测量；内容缩短后若要释放之前保留的高度，在内容更新后调用 `measure()`。行高历史按已访问的行键保存，测量缓存可能随浏览范围增长，并非只占可视窗口大小。

对于按需加载的数据，可通过 `virtualSource` 提供行数、列数及索引回调。下方示例支持加载大数据、跳转到中部或末尾，以及双向滚动。实际数据规模应结合设备内存、行高和单元格复杂度选择；服务端数据可配合远程分页使用。

虚拟滚动限制的是挂载的行列数量，不会把所有数据操作都变成窗口内计算。普通 `data` 的已加载行仍需建立索引，本地排序、筛选、分组和聚合仍会处理对应范围的数据；这些同步计算没有独立取消接口。生成源的排序、筛选和分组结果由应用或服务端提供，表格不会为此遍历全部生成行，也不会自动请求未提供的分页或懒加载子节点。

全量校验仍需逐项扫描，`maxErrors` 只限制错误数量；查找、剪贴板和图表应按各自 API 缩小范围并设置处理预算。异步操作的取消在检查点和等待处生效，不能打断正在执行的同步业务回调，也不能撤回已经完成的外部写入。测量缓存、已加载详情、变更记录和历史引用会各自占用内存；单元格上限、历史条数或浏览器 Map 容量都不代表可用行数保证。
