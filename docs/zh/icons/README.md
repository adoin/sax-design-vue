...
Sax 组件会为自身默认控件携带一小组 Carbon 回退数据，因此分页箭头、关闭按钮、选择器提示、表格操作等内部图标在未配置 Carbon 时仍能显示。显式 `iconData` 和应用注册的图标优先于回退数据；IconPicker 仅在使用该组件时加载更完整的内置默认列表。应用自行选择的图标名仍需按下文配置对应集合或 `safelist`。
