...
本记录逐项核对 `todos.md` 中 24 项新增能力，而不是用测试总数代替功能覆盖。验收维度为：默认行为、边界、受控更新、异常、卸载清理和键盘操作。纯数据算法没有 DOM 生命周期或键盘入口时标为“不适用”；其公开交互入口仍由 Table 集成测试验证。测试名称描述的是可观察结果，断言同时检查数据所有权、事件、焦点、监听器或渲染窗口，不只检查组件能够挂载。
...
上述证据满足“单元测试覆盖默认行为、边界、受控更新、异常、卸载清理及键盘操作”。它不代替真实浏览器中的主题、弹层定位和 Code/Playground 检查，也不单独证明 24 项能力同时叠加时的组合正确性；这些仍由 `todos.md` 后续两个独立验收项跟踪。
